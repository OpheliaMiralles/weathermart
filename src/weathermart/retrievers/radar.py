...
def align_to_template(other, tol_m=1000.0):
    ...
    if (
        other_stacked.sizes["cell"] == 3614996
    ):  # norm for all files from Nov 2020 onwards
        return other
    td = (
        xr.open_zarr("/lustre/storeB/users/opmir9231/nordic_radar/20250101")
        .isel(time=0)
        .coords.to_dataset()
    )
    ...
    out["lwe_precipitation_rate"] = out["lwe_precipitation_rate"].where(
        xr.DataArray(ok.astype(bool), dims=["cell"]), np.nan
    )
    out = (
        out.reindex_like(td)
        .assign_coords(lat=("cell", tlat), lon=("cell", tlon))
        .unstack("cell")
    )
    return out


def ll_to_xyz(lat_deg, lon_deg):
...
def get_old_to_new_idx(other=None, tol_m=1000.0):
    ...
    other = (
        other
        or xr.open_zarr("/lustre/storeB/users/opmir9231/nordic_radar/20200415")
        .isel(time=0)
        .coords.to_dataset()
    )
    ...
    d_m = arc * R
    ok = d_m <= tol_m
    dataframe = pd.DataFrame({"selected_index": idx, "distance": d_m, "mask": ok})
...
class NordicRadarRetriever(BaseRetriever):
    ...
    def decode_qc_flags(
        ...
        *,
        ...
        out = {}
        for bit, name in self.mapping_flags.items():
            out[name] = ((ds[flag_var] >> np.uint64(bit)) & np.uint64(1)).astype(
                "uint8"
            )
